...
# --- APP CONFIGURATION ---
app = Flask(__name__)
app.config['SECRET_KEY'] = 'a_very_secret_key_change_this' # Change this in production!
basedir = os.path.abspath(os.path.dirname(__file__))
# Ensure the instance folder exists before initializing the app
instance_path = os.path.join(basedir, 'instance')
os.makedirs(instance_path, exist_ok=True)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(instance_path, 'dsa_progress.db')
...
THEMATIC_SUBTOPICS = {
    ...
    "two-pointers": [{"name": "Two Pointers", "tags": ["Two Pointers"]}],
    "stack": [{"name": "Monotonic Stack", "tags": ["Monotonic Stack"]}, {"name": "Core Stack Problems", "tags": ["Stack"]},],
    "binary-search": [{"name": "Binary Search", "tags": ["Binary Search"]}],
    "sliding-window": [{"name": "Monotonic Queue", "tags": ["Monotonic Queue"]}, {"name": "Sliding Window", "tags": ["Sliding Window"]},],
    "linked-list": [{"name": "Linked List", "tags": ["Linked List"]}],
...
PROBLEM_LIMIT_PER_TOPIC = 75
DIFFICULTY_QUOTAS = {'Easy': 0.35, 'Medium': 0.45, 'Hard': 0.20}
...
ROADMAP_TOPIC_LEVEL = { "Arrays & Hashing": 1, "Two Pointers": 1, "Stack": 1, "Binary Search": 1, "Sliding Window": 1, "Linked List": 1, "Trees": 2, "Tries": 3, "Heap / Priority Queue": 3, "Backtracking": 2, "Intervals": 3, "Greedy": 3, "Graphs": 3, "Advanced Graphs": 4, "1-D DP": 3, "2-D DP": 3, "Bit Manipulation": 5, "Math & Geometry": 5 }
SPECIFIC_PATTERN_TAGS = set(sum([tags for name, tags in PRIMARY_TOPIC_MAP.items() if name not in ["Arrays & Hashing", "Math & Geometry"]], []))

def adjusted_like_ratio(row):
    total_votes = row['Likes'] + row['Dislikes']
    if total_votes == 0: return 0
    vote_quality = min(total_votes / 2000, 1.0)
    return (row['Likes'] / total_votes * vote_quality * 100)
def get_acceptance_rate_bonus(row):
    rate = row.get('AcceptanceRate', 0)
    if row['Difficulty'] == 'Easy' and 60 < rate < 85: return 10
    if row['Difficulty'] == 'Medium' and 40 < rate < 70: return 10
    if row['Difficulty'] == 'Hard' and 25 < rate < 50: return 10
    return 0
...
def calculate_signal_score(row):
    submission_score = math.log1p(row.get('TotalSubmissions', 0)) * 2.5
    acceptance_bonus = get_acceptance_rate_bonus(row)
    classic_bonus = 25 if row['ID'] in CLASSIC_PROBLEM_IDS else 0
    score = (row['adjusted_like_ratio'] * 0.4) + (submission_score * 0.6) + classic_bonus + acceptance_bonus
...
def load_data():
    ...
    try:
        ...
        df.loc[df['Title'].str.contains('Interval', case=False), 'Topics'] = df.loc[df['Title'].str.contains('Interval', case=False), 'Topics'].apply(lambda x: x + ['Intervals'])
        df['Difficulty'] = pd.Categorical(df['Difficulty'], categories=['Easy', 'Medium', 'Hard'], ordered=True)
        df['TotalSubmissions'] = df['TotalSubmissions'].fillna(0).apply(parse_submissions)
        df['SimilarQuestions'] = df['Similar Questions'].fillna('[]').apply(parse_similar_questions)
        df['adjusted_like_ratio'] = df.apply(adjusted_like_ratio, axis=1)
        df['prerequisite_tier'] = df['Topics'].apply(lambda tl: max([TOPIC_TIERS.get(t, 99) for t in tl] or [99]))
        df['signal_score'] = df.apply(calculate_signal_score, axis=1)
        DATA = df
        print(f"✅ LeetCode data loaded and scored. Total problems: {len(DATA)}")
    except FileNotFoundError:
        print("❌ ERROR: leetcode_with_submissions.csv not found.")
        DATA = pd.DataFrame()
def get_curated_problems_for_topic(topic_slug):
    ...
    max_allowed_tier = ROADMAP_TOPIC_LEVEL.get(graph_topic_name, 99)
    primary_tags = PRIMARY_TOPIC_MAP.get(graph_topic_name, [])
    mask = DATA['Topics'].apply(lambda topics: any(t in primary_tags for t in topics))
    base_df = DATA[mask & (DATA['prerequisite_tier'] <= max_allowed_tier)].copy()
    if graph_topic_name == "Arrays & Hashing":
        base_df = base_df[~base_df['Topics'].apply(lambda topics: any(t in SPECIFIC_PATTERN_TAGS for t in topics))]
    elif graph_topic_name not in ["Math & Geometry"]:
        other_specific_tags = SPECIFIC_PATTERN_TAGS - set(primary_tags)
        base_df = base_df[~base_df['Topics'].apply(lambda topics: any(t in other_specific_tags for t in topics))]
    ...
    stratified_dfs = []
    base_df = base_df.sort_values(by='signal_score', ascending=False)
    for difficulty, quota in DIFFICULTY_QUOTAS.items():
        limit = math.ceil(PROBLEM_LIMIT_PER_TOPIC * quota)
        difficulty_group = base_df[base_df['Difficulty'] == difficulty]
        stratified_dfs.append(difficulty_group.head(limit))
    if not stratified_dfs: return []
    curated_df = pd.concat(stratified_dfs).drop_duplicates(subset=['ID'])
    thematic_curriculum = THEMATIC_SUBTOPICS.get(topic_slug, [{"name": graph_topic_name, "tags": primary_tags}])
    final_ordered_problems = []
    processed_ids = set()
    for theme in thematic_curriculum:
        theme_tags = set(theme['tags'])
        theme_problems_mask = curated_df['Topics'].apply(lambda topics: any(tag in theme_tags for tag in topics))
        theme_df = curated_df[theme_problems_mask & ~curated_df['ID'].isin(processed_ids)]
        if not theme_df.empty:
            theme_df_sorted = theme_df.sort_values(by=['Difficulty', 'signal_score'], ascending=[True, False])
            final_ordered_problems.extend(theme_df_sorted.to_dict('records'))
            processed_ids.update(theme_df_sorted['ID'].tolist())
...
def index():
    ...
    topic_problems_map = {}
    for slug in SLUG_TO_NAME_MAP.keys():
        curated_problems = get_curated_problems_for_topic(slug)
        topic_problems_map[slug] = [p['ID'] for p in curated_problems]
...
def topic_page(topic_slug):
    if DATA.empty: abort(500)
    problems_to_show = get_curated_problems_for_topic(topic_slug)
    graph_topic_name = SLUG_TO_NAME_MAP.get(topic_slug)
...
def get_progress():
    ...
    solved_map = {
        str(p.problem_id): {'solved_at': p.solved_at.isoformat()}
...
def toggle_progress():
    ...
    if problem_id is None or is_solved is None:
