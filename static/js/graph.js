document.addEventListener('DOMContentLoaded', () => {
    ...
    tsParticles.load("particles-bg", {
        ...
        interactivity: { events: { onHover: { enable: true, mode: ["repulse", "bubble"] }, }, modes: { repulse: { distance: 100, duration: 0.4 }, bubble: { distance: 200, size: 2.5, opacity: 0.8 }, }, },
        particles: {
            color: { value: ["#ffffff", "#a78bfa", "#2dd4bf"] },
            links: { color: "rgba(167, 139, 250, 0.2)", distance: 150, enable: true, opacity: 0.2, width: 1, },
            move: { direction: "bottom-right", enable: true, outModes: "out", random: true, speed: 0.3, straight: false, },
    ...
    const nodes = [ { id: "AH", name: "Arrays & Hashing",    x: 0,    y: -900 }, { id: "TP", name: "Two Pointers",        x: -500, y: -700 }, { id: "S",  name: "Stack",               x: 500,  y: -700 }, { id: "BS", name: "Binary Search",       x: -750, y: -500 }, { id: "SW", name: "Sliding Window",      x: -250, y: -500 }, { id: "LL", name: "Linked List",         x: 500,  y: -500 }, { id: "T",  name: "Trees",               x: -250, y: -300 }, { id: "Tr", name: "Tries",               x: -800, y: -50 }, { id: "HPQ",name: "Heap / Priority Queue", x: -250, y: -50 }, { id: "BT", name: "Backtracking",        x: 550,  y: -50 }, { id: "I",  name: "Intervals",           x: -850, y: 200 }, { id: "G",  name: "Greedy",              x: -500, y: 200 }, { id: "AG", name: "Advanced Graphs",     x: -100, y: 200 }, { id: "Gr", name: "Graphs",              x: 350,  y: 200 }, { id: "DP1",name: "1-D DP",              x: 750,  y: 200 }, { id: "DP2",name: "2-D DP",              x: 300,  y: 450 }, { id: "BM", name: "Bit Manipulation",    x: 750,  y: 450 }, { id: "MG", name: "Math & Geometry",     x: 500,  y: 650 } ];
    const linksData = [ { source: "AH", target: "TP" }, { source: "AH", target: "S" }, { source: "TP", target: "BS" }, { source: "TP", target: "SW" }, { source: "TP", target: "LL" }, { source: "BS", target: "T" }, { source: "SW", target: "T" }, { source: "LL", target: "T" }, { source: "T", target: "Tr" }, { source: "T", target: "HPQ" }, { source: "T", target: "BT" }, { source: "HPQ", target: "I" }, { source: "HPQ", target: "G" }, { source: "HPQ", target: "AG" }, { source: "BT", target: "Gr" }, { source: "BT", target: "DP1" }, { source: "Gr", target: "AG" }, { source: "Gr", target: "DP2" }, { source: "DP1", target: "DP2" }, { source: "DP1", target: "BM" }, { source: "DP2", target: "MG" }, { source: "BM", target: "MG" } ];
    ...
    const nodeBgGradient = defs.append("linearGradient").attr("id", "node-bg-gradient").attr("x1", "0%").attr("y1", "0%").attr("x2", "100%").attr("y2", "100%");
    ...
    const barYOffset = 26;
    const barPadding = 28;
    function generatePath(d) { const sourceY = d.source.y + (nodeHeight / 2) - 4; const targetY = d.target.y - (nodeHeight / 2) - 4; return d3.linkVertical()({ source: [d.source.x, sourceY], target: [d.target.x, targetY] }); }
    ...
    const node = g.append("g").selectAll("g").data(nodes).join("g").attr("transform", d => `translate(${d.x},${d.y})`).on("click", (event, d) => { if (!isUnlocked(d.id)) { tooltip.html(`Complete 50% of<br>prerequisite topics to unlock!`).style("opacity", 1); return; }; window.location.href = `/topic/${slugMap[d.id]}`; });
    node.append("rect").attr("class", "node-background").attr("width", nodeWidth).attr("height", nodeHeight).attr("rx", 32).attr("ry", 32).style("fill", "url(#node-bg-gradient)").attr("stroke", "#a78bfa").attr("stroke-width", 2.5).style("filter", "url(#node-glow)").attr("x", d => -nodeWidth(d) / 2).attr("y", -nodeHeight / 2);
    node.append("rect").attr("class", "progress-bar-bg").attr("width", d => nodeWidth(d) - barPadding * 2).attr("height", barHeight).attr("rx", barHeight / 2).attr("ry", barHeight / 2).style("fill", "rgba(0,0,0,0.4)").attr("x", d => -(nodeWidth(d) - barPadding * 2) / 2).attr("y", barYOffset);
    node.append("rect").attr("class", "progress-bar-fill").attr("width", 0).attr("height", barHeight).attr("rx", barHeight / 2).attr("ry", barHeight / 2).style("fill", "url(#progress-gradient)").style("filter", "drop-shadow(0 0 5px rgba(45, 212, 191, 0.5))").attr("x", d => -(nodeWidth(d) - barPadding * 2) / 2).attr("y", barYOffset);
    node.append("text").attr("fill", "#e5e7eb").attr("font-weight", "500").attr("font-size", "24px").attr("text-anchor", "middle").attr("y", -6).text(d => d.name).style("pointer-events", "none");
    
    // --- 4. PROGRESS & UNLOCKING LOGIC ---
    let solvedMapCache = new Map();

    async function fetchAndInitializeProgress() {
        try {
            const url = `/api/progress?t=${new Date().getTime()}`;
            const response = await fetch(url);
            if (!response.ok) throw new Error(`Network response was not ok: ${response.statusText}`);
            const solvedData = await response.json();
            solvedMapCache = new Map(Object.entries(solvedData));
        } catch (error) {
            console.error("Failed to fetch user progress:", error);
            solvedMapCache = new Map();
        }
        updateProgressBars();
    }

    function getSolvedMap() { return solvedMapCache; }

    function calculateProgress(nodeId) {
        const solvedMap = getSolvedMap();
        const solvedIds = new Set(Array.from(solvedMap.keys()));
        const slug = slugMap[nodeId];
        if (!topicProblemsMap[slug]) return { solved: 0, total: 0, percentage: 0 };
        const allProblemIds = topicProblemsMap[slug].map(String);
    ...
    function isUnlocked(nodeId) {
        ...
        return parentIds.every(parentId => calculateProgress(parentId).percentage >= 50);
    }
    ...
    function updateNodeStates() {
        node.each(function(d) {
            ...
            sel.classed('locked', !unlocked).style('cursor', unlocked ? 'pointer' : 'not-allowed');
            
            // --- THIS IS THE GUARANTEED FIX ---
            // Removed ALL .transition() calls from this function. Updates are now instant.
            sel.select(".node-background")
                .style("fill", unlocked ? "url(#node-bg-gradient)" : "#374151")
                .attr("stroke", unlocked ? "#a78bfa" : "#4b5563");
            
            sel.attr('opacity', unlocked ? 1.0 : 0.6);
